...
class GaborNN8Layer(nn.Module):
    def __init__(self):
        super(GaborNN8Layer, self).__init__()
        ...
        self.c5 = nn.Conv2d(512, 1024, kernel_size=(1, 1))
        self.c6 = nn.Conv2d(1024, 1024, kernel_size=(1, 1))
    ...
    def forward(self, x):
        ...
        x = nn.Dropout2d()(x)
        x = self.bn3(x)
        x = F.max_pool2d(F.leaky_relu(self.c5(x)), kernel_size=1)
        x = F.max_pool2d(F.leaky_relu(self.c6(x)), kernel_size=1)
        x = nn.Dropout2d()(x)
        x = x.view(-1, 1024 * 7 * 7)
